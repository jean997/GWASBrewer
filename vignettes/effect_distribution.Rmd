...
hist(std_effects2[!std_effects2 == 0], breaks = 30, freq = F, main = "Standardized Direct Effects Trait 1")
```

Below is some code that uses combines some of the effect size speicification utilities to simulate GWAS data for only one LD block for two traits. The first trait will have two effect variants that have equal effect size. The second trait has two effect variants with different effect sizes. One pair of variants co-localizes. Simulations of this type could be used to test co-localization methods.  Since we are looking at only one block, we will set the heritability of the block to be a reasonable amount of heritability provided by two variants, 6e-4. 
...
plot(sim_dat7$direct_SNP_effects_joint[,1])
abline(v = c(75, 150))
plot(sim_dat7$direct_SNP_effects_joint[,2])
abline(v = c(75, 130))

## Plotting marginal effects
plot(sim_dat7$direct_SNP_effects_marg[,1])
abline(v = c(75, 150), h = 0)
plot(sim_dat7$direct_SNP_effects_marg[,2])
