...
+ `G`: The matrix specifying direct effects in the linear SEM. 

There are additional optional arguments:

+ `R_E`: Matrix giving correlation between environmental components of each trait (see section below for more details). This option is ignored if there is no sample overlap.
+ `R_LD` and `snp_info`: Options used if GWAS data should have LD
...
+ `beta_joint` and `beta_marg`: Total associations (including those mediated by other traits). `beta_marg` is the expected value of `beta_hat`.
...
Variables $Z_1, \dots, Z_{K-2}$ can have effects either on or from $X$, $Y$ or both. So
...
The program will give an error if the user tries to specify a mediator between $Y$ and $X$ as well as a non-zero
effect of $X$ on $Y$ as this would create a cycle.
...
effect of $X$ on $Y$ is negative. The direction parameters `dri_xz` and `dir_yz` are vectors of 1 or -1 with length $K-2$. These specify the direction of effect between $Z_k$ and $X$ or $Y$ with 1 indicating an effect on $X$ or $Y$ and -1 indicating an effect to $X$ or $Y$. 
...
The true SNP effects are also stored in the simulation object. The `direct_SNP_effects_joint` and `beta_joint` tablse give the direct and total SNP effects. The `_joint` ending here indicates that these variables store the expected *joint* association, conditional on all other variants. GWAS effect estimates measure the marginal association, so we also store the expected direct and total *marginal* associations in 
`direct_SNP_effects_marg` and `beta_marg`. When all variants are independent (no LD),  the expected joint and marginal associations are the same. However, if we had generated data with LD, this would not be the case (see below for more on LD). With or without LD, `beta_marg` is always the expected value of `beta_hat`, that is `beta_marg` is the association we would measure with infinite GWAS sample size. 
...
Because SNP effects are generated from a point-normal distribution, most SNPs are non-zero in at most one of $Z$ or $W$. However, there are a handful of variants that have direct effects on both. This is "sporadic pleiotropy" and can be prevented with `sporadic_pleiotropy=FALSE` if it is not desired. 

Next we plot the total SNP effects on $Z$ vs the total SNP effects on $W$. Because $Z$ has a causal effect on $W$, all variants with effects on $Z$ also affect $W$. The line in the plot has slope equal to the total effect of $Z$ on $W$. The majority SNPs that have non-zero effect on $Z$ fall exactly on this line. With `sporadic_pleiotropy= FALSE`, all of the variants with non-zero effect on $Z$ would fall on this line. There are also some variants that have zero effect on $Z$ but non-zero effect on $W$. These are the variants with non-zero direct effect on $W$ but no direct effect on $Z$. 
...
Finally, we can verify that the observed GWAS effect estimate is a noisy measurement of `beta_marg` (in this case equal to `beta_joint`)
...
By default, when there is no LD, `sim_mv` returns standardized effects and standardized effect estimates. These are the estimates we would obtain if genotypes were scaled to have population variance 1 rather than being coded as 0, 1, and 2. The relationship between standardized and non-standardized effect estimates is simply $\hat{\beta}_j^{(non-std)} = \hat{\beta}_j^{(std)}/sd(g_j)$ where $j$ indexes variants, $sd(g_j)$ is the population standard deviation of genotype $j$. Similarly the relationship between the variance of standardized and non-standardized effect estimates is $se\left(\hat{\beta}_j^{(non-std)}\right) = se\left(\hat{\beta}_j^{(std)}\right)/sd(g_j)$. 

In our previously generated `sim_dat1`, every effect estimate has the same standard error. This is because the standard error of $\hat{\beta}_j^{(std)}$ is approximately $1/\sqrt{N}$ and we used a common sample size across all traits. Non-standardized effect estimates will have different standard errors because these also depend on variant allele frequency. 
...
$\hat{z}_{1j}$ and $\hat{z}_{2j}$, $z$-scores for variant $j$ in study 1 and study 2, is approximates $\frac{N_c}{\sqrt{N_1 N_2}} \rho_{1,2}$ where $\rho_{1,2}$ is the trait correlation. 
...
samples are in all studies or if different sets overlap between different pairs.
Prior to this, our commands have all specified that the GWAS were performed in non-overlapping samples.
...
The `R` object will always be the identity if there is no sample overlap. 
