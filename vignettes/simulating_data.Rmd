...
We also specify an envrionmental correlation matrix in which the pairwise correlation in environmental effects for any pair of traits is 0.4. `R_E` defaults to the identity if it is not provided.

```{r}
set.seed(10)
R_E <- matrix(0.4, nrow = 4, ncol = 4)
diag(R_E) <- 1
N <- matrix(60000, nrow = 4, ncol = 4)
diag(N) <- 30000
sim_dat3 <- sim_mv(G = G,
                  N = N, J = 1e5, R_E = R_E,
                  h2 = c(0.3, 0.3, 0.5, 0.4), 
