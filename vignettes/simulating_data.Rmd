...
By default, when there is no LD, `sim_mv` returns standardized effects and standardized effect estimates. These are the estimates we would obtain if genotypes were scaled to have population variance 1 rather than being coded as 0, 1, and 2. The relationship between standardized and non-standardized effect estimates is simply $\hat{\beta}_j^{(non-std)} = \hat{\beta}_j^{(std)}/sd(g_j)$ where $j$ indexes variants, $sd(g_j)$ is the population standard deviation of variant $j$. Similarly the relationship between the variance of standardized and non-standardized effect estimates is $se\left(\hat{\beta}_j^{(non-std)}\right) = se\left(\hat{\beta}_j^{(std)}\right)/sd(g_j)$. 
