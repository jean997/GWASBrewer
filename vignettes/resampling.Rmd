...
`resample_inddata` uses the `hapsim` package to generate individual level data. Currently, the `hapsim` package needs to be explicitly loaded before using the function. 
...
The resampling functions in `simGWAS` assume that effect sizes in the new population are the same as in the old population on the genotype and phenotype scale given. If you are changing populations, it is much more sensible to assume that effect sizes are constant on the per-allele scale than on the per-genotype SD scale. This means that if you want to regenerate data from a different population, it is advisable to make sure that the input data have `geno_scale` equal to "allele". One consequence of differing allele frequencies and LD structure is that the total genetic variance will be different in the new population than in the old. By default, the resampling functions will assume that the environmental variance is the same in the two populations. This means that the overall variance of the phenotype in the new population will probably not be 1. The resampling functions will **not** rescale the phenotype in the new population because this would mean that the phenotypes in the two populations had different units and were not comparable. 

As a simple extreme example to illustrate this issue, suppose that $Y$ has one causal variant that explains 30% of its variance in population A. Lets say this variant has allele frequency 0.25 in population A. We can compute that if $Y$ is scaled to have unit variance in population A, then each alternate allele of the causal variant increases $Y$ by $0.894$ units and the environmental variance is $0.7$. We got the effect size by computing $\sqrt{0.3/(2\cdot 0.25\cdot(1-0.25))}$ which gives us the desired variance explained by the genotype. Now suppose that in population B, the effect size and environmental contribution to the trait are the same but the allele frequency is only 0.01. In this case, when put on the same scale that gave a unit variance in population A, $Y$ only has a variance of $(0.894^2)\cdot(2 \cdot 0.01\cdot(1-0.01)) + 0.7 = 0.716$. 
...
Although the default behavior is to keep `Sigma_E` constant, we can change this using the arguments `new_env_var`, and one of `new_R_E` or `new_R_obs`. These are options for both `resample_sumstats` and `resample_inddata`. The parameter `new_env_var` is a scalar or vector of lenth $M$ giving the environmental variance in the new population. If supplied, `new_R_E` is the environmental correlation in the new population and `new_R_obs` is the total trait correlation in the new population. Only one of these two parameters can be supplied. 
...
We can also change the scale of the outcome.
